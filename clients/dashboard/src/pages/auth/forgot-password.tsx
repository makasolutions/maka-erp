...
export function ForgotPasswordPage() {
  ...
  return (
    <AuthShell
      footer={
        <span>
          {t("auth.forgotPasswordForm.rememberedIt")}{" "}
          <Link
            to="/login"
    ...
    >
      {submitted ? (
        <div className="fsh-enter space-y-5 text-center">
          ...
          <div>
            <AuthHeadline lead={t("auth.forgotPasswordForm.checkInboxTitle")} accent={t("auth.forgotPasswordForm.checkInboxAccent")} />
          ...
          <ul className="space-y-1.5 text-left text-[12.5px] text-[var(--color-muted-foreground)]">
            <li className="flex items-start gap-2">
              ...
              {t("auth.forgotPasswordForm.didntGetIt")}
            </li>
            <li className="flex items-start gap-2">
              <Check className="mt-0.5 size-3.5 shrink-0 text-[var(--color-success)]" />
              {t("auth.forgotPasswordForm.stillNothing")}
            </li>
          </ul>
        ...
        <>
          <div className="mb-6 sm:mb-8">
            <AuthHeadline lead={t("auth.forgotPasswordForm.title")} accent={t("auth.forgotPasswordForm.accent")} />
            <p className="text-[13px] text-[var(--color-muted-foreground)]">
              {t("auth.forgotPasswordForm.desc")}
            </p>
          </div>
              ...
              >
                {mutation.isPending ? (
                  <>
                    ...
                    <span>{t("auth.forgotPasswordForm.sendingLink")}</span>
                  </>
                ) : (
                  <>
                    <span>{t("auth.forgotPasswordForm.sendLink")}</span>
