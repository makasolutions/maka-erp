...
export function MentionPicker({
  ...
  return (
    <div
      role="listbox"
      aria-label={t("mention.suggestionsAriaLabel")}
      className={cn(
    ...
    >
      ...
      <ul className="max-h-[240px] overflow-y-auto p-1">
        {candidates.map((user, i) => {
          const display = renderName(user, t("unnamed"));
...
function renderName(u: UserDto, unnamedLabel: string): string {
  const full = [u.firstName, u.lastName].filter(Boolean).join(" ").trim();
  return full || u.userName || u.email || unnamedLabel;
}
