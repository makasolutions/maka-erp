...
/// <summary>
/// Bounds an autogenerated key to a column's max length, preserving uniqueness by
/// appending a short content hash when truncation is needed. Avoids "value too long"
/// DbUpdateExceptions when prefixing a tenant id onto an already-long SKU/slug.
/// </summary>
internal static class GlobalKey
{
    public static string Bound(string raw, int maxLength)
    {
        raw = raw.ToLowerInvariant();
        if (raw.Length <= maxLength) return raw;
        // Stable, non-cryptographic FNV-1a hash → 6 hex chars; just needs to disambiguate truncations.
        uint h = 2166136261u;
        foreach (char c in raw) { h = (h ^ c) * 16777619u; }
        string hash = (h & 0xFFFFFF).ToString("x6", System.Globalization.CultureInfo.InvariantCulture);
        return string.Concat(raw.AsSpan(0, maxLength - 7), "-", hash);
    }
}
    ...
    : ICommandHandler<PublishProductToGlobalCommand, Guid>
{
    public async ValueTask<Guid> Handle(PublishProductToGlobalCommand command, CancellationToken cancellationToken)
    {
        ...
        string globalSlug = GlobalKey.Bound($"{ownerId}-{p.Slug}", 220);
        string globalSku = GlobalKey.Bound(sku is null ? $"{globalSlug}-default" : $"{ownerId}-{sku}", 64);
        string name = p.Name;
    ...
    : ICommandHandler<AdoptGlobalProductCommand, Guid>
{
    ...
    public async ValueTask<Guid> Handle(AdoptGlobalProductCommand command, CancellationToken cancellationToken)
    {
        ...
        string slug = GlobalKey.Bound(SlugHelper.Build(null, snap.Name), 220);
        ...
        if (slugTaken) slug = GlobalKey.Bound($"{slug}-{Guid.NewGuid().ToString("N")[..6]}", 220);

        var product = Product.Create(snap.Name, slug, snap.Type == ProductType.Variable ? ProductType.Simple : snap.Type,
            shortDescription: snap.ShortDescription, defaultSku: GlobalKey.Bound($"{slug}-default", 64));
